...
public:
  ...
  std::string getTestFilename(const std::string &suffix, unsigned id, double time);
...
std::string KleeHandler::getTestFilename(const std::string &suffix, unsigned id, double time) {
  std::stringstream filename;
  filename << "test" << std::setfill('0') << std::setw(6) << id << '-' << time << '.' << suffix;
  return filename.str();
}
...
void KleeHandler::processTestCase(const ExecutionState &state,
  ...
  if (!NoOutput) {
    ...
    if (success) {
      ...
      kTest_toFile(&b, getOutputFilename(getTestFilename("ktest", id, util::getWallTime() - kleeStart)).c_str());
      std::ofstream ofs;
